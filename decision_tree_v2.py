...
class Decision_tree:
    ...
    def get_best_node_in_layers(tree, symbol):
        ...
        # Use multiprocessing to parallelize layer2_decision
        pool = multiprocessing.Pool(processes=multiprocessing.cpu_count())
        results = []

        # Define the layer2_decision function that will run in parallel
        def parallel_layer2_decision(a1, symbol):
            best = -symbol * 999999
            best_move_ = None
            a1.expand(symbol)

            for layer1 in a1.next_moves:
                layer2 = layer1.get_best_move(-symbol)
                total = layer2.evaluate + layer1.evaluate
                if total * symbol > best * symbol:
                    best = total
        ...
        for a1 in tree.next_moves:
            if a1.is_win():
                return a1
            a2 = parallel_layer2_decision(a1, -symbol)
            if a2.is_win():
                continue
            a2.expand(symbol)
            for a3 in a2.next_moves:
                a4 = parallel_layer2_decision(a3, -symbol)
                score = a1.evaluate + a2.evaluate + a3.evaluate + a4.evaluate
                results.append((score, a1))

        # Close the pool and wait for all processes to complete
        pool.close()
        pool.join()

        # Find the move with the best score among all results
